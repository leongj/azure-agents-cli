...
import { vectorStoresList, vectorStoreShow } from '../commands/vectorstores.js';
...
export async function dispatch(ctx) {
  ...
  switch (main) {
    ...
    case 'vectorstores':
      ...
      if (sub === 'files') {
        // syntax: aza vs files list <vectorStoreId>
        //         aza vs files show <vectorStoreId> <fileId>
        if (sub2 === 'list') return filesList(ctx, sub3);
        if (sub2 === 'show') return fileShow(ctx, sub3, sub4);
        throw usageError('Usage: aza vs files (list <vectorStoreId>|show <vectorStoreId> <fileId>)');
      }
      throw usageError('Usage: aza vs (list|show <vectorStoreId>|files list <vectorStoreId>|files show <vectorStoreId> <fileId>)');
    case 'file': // deprecated singular alias
    case 'files':
      if (sub === 'list') return filesList(ctx, sub2);
      if (sub === 'show') return fileShow(ctx, sub2, sub3);
      throw usageError('Usage: aza files (list <vectorStoreId>|show <vectorStoreId> <fileId>)');
    default:
